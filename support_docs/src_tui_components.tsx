...
export function StatusDot({ status }: { status: Status }) {
  ...
  return <Text color={statusColor(theme, status)}>{STATUS_SYMBOL[status]}</Text>
}
...
export function ProgressBar({
  ...
  const color = percent >= warnAt ? theme.danger : percent >= 0.6 ? theme.warning : theme.success
  ...
  return (
    <Box borderStyle="single" borderColor={theme.border} paddingX={1}>
      {tabs.map((tab, i) => {
        ...
        const color = isActive ? agentTabColor(theme, tab.name) : theme.textTertiary
        return (
          <Box key={tab.name} marginRight={2}>
            <Text
              color={color}
              bold={isActive}
              underline={tab.isCustom}
              dimColor={!isActive}
            >
              {tab.displayName}
            </Text>
...
export function StatusBar({
  ...
  return (
    <Box justifyContent="space-between" paddingX={1}>
      ...
      <Text color={agentTabColor(theme, activeAgentName)}>{activeAgentName} ▸</Text>
    </Box>
  )
}
