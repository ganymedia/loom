...
- Completed Developer responses render as labeled, bordered `Developer summary` blocks; strict JSON-fenced response envelopes stream and render only their operator-facing content, while malformed tool-envelope output fails with a fixed message instead of displaying tool fields. Live streaming and other agents' completed output retain their existing presentation.
