...
- Interactive TTY sessions use Ink Static for completed scrollback while retaining a live streaming assistant region and status/input regions; persistent Ink, instant Tab cycling, and token streaming are supported, and now include resize reflow and clean Ctrl+C/Escape exits. Typing `/` as the first input character shows an inline list of available built-in agent commands and existing session commands that filters live, case-insensitively, by executable command prefix as characters are typed. When the popup is visible, use Up/Down to move a marked selection (with wraparound) and Enter to submit the exact command; bare Tab still cycles agents globally.
