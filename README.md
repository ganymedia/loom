...
- Explicit Developer read-then-write requests may run up to three permitted model/tool rounds, with a 60-second timeout on each backend request and visible `Applying tool results…` progress between rounds. Up to 64 KiB of tool results can be returned as untrusted project data to the same configured backend so a subsequent write can use the read result; this data is not logged. Do not use this flow for CUI unless that backend/data boundary is approved.
...
- Interactive TTY sessions use a terminal-height-aware persistent Ink conversation viewport with fixed agent tabs, status, and input regions; long output remains inside the conversation pane instead of displacing or crossing the composer. PageUp/PageDown navigate measured history by one viewport, Ctrl+Home jumps to the oldest available row, and Ctrl+End returns to the latest activity; a fixed history notice appears while scrolled. Completed output is theme-muted and dimmed so the live exchange remains the clear focus. Assistant Markdown renders as styled headings, emphasis, lists, inline code, tables, quotes, and recognized syntax-highlighted fenced code; raw HTML remains inert text and terminal control characters are neutralized. Successful file-writer calls render bounded, red/green before-and-after line diffs in the interactive view; oversized changes show an explicit omission notice, while non-TTY output retains its concise tool-result format. Persistent Ink, instant Tab cycling, and token streaming are supported, and now include resize reflow and clean Ctrl+C/Escape exits. Interactive prompts show an animated agent-specific Thinking indicator immediately after submission and remove it when the first streamed token arrives or the turn ends/errors. Existing tool execution has a distinct animated amber action indicator such as `Reading file`, `Writing file`, `Running command`, or `Checking repository`; all built-in agents receive the response-envelope contract for their permitted tools, mixed work retains a bounded tool-count fallback, and no names, arguments, paths, content, or results enter the lifecycle signal. Typing `/` as the first input character shows an inline list of available built-in agent commands and existing session commands that filters live, case-insensitively, by executable command prefix as characters are typed. When the popup is visible, use Up/Down to move a marked selection (with wraparound) and Enter to submit the exact command; Escape closes an open slash-command popup without submitting or clearing typed input, while Escape when no popup is open still exits the interactive session. Subsequent typing reopens live discovery for preserved slash input. Bare Tab still cycles agents globally.
- Interactive non-diff tool results use a green `✓` success or red `✕` failure header, with returned file/output text separated into a bordered neutral-color body; non-TTY tool-result text is unchanged. Successful file-write diffs are followed by an explicit green completion row.
- Completed Developer responses render as labeled, bordered `Developer summary` blocks; strict JSON-fenced response envelopes stream and render only their operator-facing content. Empty, malformed, or action-announcement responses without a tool call receive up to two bounded repair attempts, then fail with a fixed message instead of displaying tool fields. Live streaming and other agents' completed output retain their existing presentation.
- Wide, tall interactive TTY sessions add a rendering-only right panel with a hashed session label, directory basename, explicit runtime state, up to five pending plan tasks, and the package version. The panel hides completely on smaller terminals and is never loaded for piped input or `--prompt`. Selecting `loom-light` or `loom-dark` paints the full TUI canvas consistently while message blocks remain separated by borders rather than independent fills.
- Interactive input uses a bordered `Message` composer with a one-cell blinking cursor that paints the current character without shifting text and uses its own cell only at end of input; submitted prompts remain in bordered, labeled `You` blocks that are structurally distinct from assistant output. Plain Enter submits and Ctrl+J inserts a newline; Shift+Enter also inserts a newline only when the terminal reports the modifier. Left/Right move by character, and Ctrl+Left/Ctrl+Right or the terminal-equivalent Meta shortcuts move by word. Pasted multi-line text remains one prompt, and the input area displays the submit/newline key hints. Outside the slash popup, Up/Down traverse up to 100 process-local submitted entries and restore the current unsent draft after the newest entry.
- Interactive TTY sessions enable bounded SGR mouse tracking so the wheel scrolls conversation rows and clicks are consumed rather than inserted into the composer. PageUp/PageDown and Ctrl+Home/Ctrl+End provide keyboard scrolling; Up/Down intentionally remain prompt-history keys. Mouse tracking is disabled when the session exits.
- When reported token usage reaches 80% of the active context limit and automatic handoff generation succeeds, interactive sessions show an amber `handoff saved` notice in the pinned status bar. Successful `/recall <query>` commands show `prior context`; non-TTY output retains its existing handoff message. If no indexed history exists, it shows `Recall unavailable: no indexed prior-session history exists.`; if history exists but `store.embeddingBackend` is absent, it shows `Recall unavailable: configure store.embeddingBackend.`; other failures show a generic sanitized message.
...
- Broad configurable Bash access and bounded project-tree discovery remain deferred under FR-006 pending a dedicated permission architecture and security review; recursive shell listing remains denied. The public Foundry marketplace, package managers beyond `install.sh`, automated release CI, and telemetry are not included.
