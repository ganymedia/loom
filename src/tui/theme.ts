...
export const loomDark: Theme = {
  ...
  name: "LOOM Dark",
  description:
    "Default theme. Muted status colors, restrained accent, dense mono hierarchy.",
...
export const highContrast: Theme = {
  ...
  name: "High Contrast",
  description:
    "Maximized contrast and perceptual separation between status colors.",
...
export const monochrome: Theme = {
  ...
  name: "Monochrome",
  description:
    "Minimal color. Status conveyed primarily through symbols, not hue.",
...
export function resolveTheme(themeId: string | undefined): Theme {
  ...
  if (!theme) {
    console.warn(
      `Unknown theme "${themeId}", falling back to ${DEFAULT_THEME_ID}`,
    );
