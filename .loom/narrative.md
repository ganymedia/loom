...
## 2026-08-07 — Complete installation and release documentation

Decision: expand the product README installation section with supported systems, prerequisites, default destination, version verification, PATH recovery, version pinning, custom destinations, upgrade behavior, uninstall behavior, and advanced-host guidance instead of sending ordinary users directly into the release runbook. Correct `CHANGELOG.md` to date the published 0.1.0 release and state that security review and live publication are complete while independent external testing remains pending. Correct `USER_TESTING.md` to require the published binary's redacted `endpoint: "[configured]"` output rather than an exposed backend URL. A stale local `dist/loom` binary initially contradicted that output, but the authenticated published 0.1.0 asset confirmed redaction; the repository had also reverted to private and caused unauthenticated 404s, so verification was blocked until the operator restored public visibility. After restoration, a fresh unauthenticated pinned install into a custom temporary directory passed and printed `0.1.0`. Typecheck, lint, and focused reviews of all three documents passed. These operator-requested corrections do not complete or advance `t7-4-2`; Phase 7 remains active with 3 tasks remaining, and Phase 6 remediation `m6-6` remains merged and complete with 0 tasks remaining.

## 2026-08-07 — Installation-documentation handoff summary

Phase 6 remediation `m6-6` is merged and complete with 0 tasks remaining. Phase 7 is merged with 3 tasks remaining and remains active at `m7-4` / `t7-4-2`; these operator-requested documentation corrections did not advance the plan. This session committed the complete installation guidance, published changelog status, and redacted config-testing expectation as `a7733d2`. Unauthenticated live installation passed after public repository visibility was restored, the published binary confirmed config redaction, and typecheck, lint, and three focused documentation reviews passed. The next action is to ask the operator whether external contributions are accepted before writing the `t7-4-2` policy. No session changes remain uncommitted before this handoff update; only pre-existing operator support documents, transcript, and release-helper files remain unstaged.
...
## 2026-08-13 — Persistent Ink root blocked on compiled PTY input

Decision: do not complete or commit `t8-1-1` after the persistent Ink root passed typecheck, lint, focused TUI tests, and 224 of 225 full-suite tests but failed the existing compiled first-run PTY regression. The implementation preserves the shared agent-switching, tool-call, conversation-history, redaction, token-accounting, and handoff controller and gives real TTY sessions one Ink render tree plus an Ink-owned input queue. Three attempts established that the PTY sees the follow-up prompt and sends `/exit`, yet the process remains blocked until terminated with status `-15`; replacing React-state-only input with a synchronous ref did not resolve it. Phase 8 remains merged with 29 tasks remaining at `m8-1` / `t8-1-1`, and v0.1.1 has not been released. The working tree retains the uncommitted blocked implementation and pre-existing operator/support files; the next session must trace whether Return reaches the submission callback and whether the queue wakes its pending iterator without logging terminal content.

## 2026-08-13 — Persistent Ink render-tree foundation

Decision: complete `t8-1-1` after tracing the compiled PTY hang to Ink 7 coalescing `/exit` and its line ending into one `useInput` chunk rather than emitting a separate Return event. The input component now consumes embedded CR, LF, and CRLF boundaries while retaining incomplete input, so delayed compiled-terminal input exits cleanly without exposing input content in diagnostics. Real interactive TTY sessions use one alternate-screen Ink render tree backed by an observable view store and Ink-owned input queue; scripted and injected-output paths retain deterministic rendering. Existing agent switching, tool-call output, conversation history, redaction, token accounting, handoff ingestion, and automatic handoff behavior remain unchanged. Typecheck, lint, focused compiled-PTY and TUI verification, and all 226 tests passed. README now describes only the persistent foundation and explicitly leaves instant keybindings, streaming, pinned scrollback/input regions, and full resize/exit handling to the remaining `m8-1` tasks. Phase 8 advances to `t8-1-2` with 28 tasks remaining; v0.1.1 is not released.

## 2026-08-13 — Persistent Ink task handoff summary

Phase 8 is merged with 28 tasks remaining and active at `m8-1` / `t8-1-2`; v0.1.1 has not been released. This session completed and committed `t8-1-1` as `50fa402` after the compiled PTY, typecheck, lint, and all 226 tests passed. The persistent render-tree foundation preserves existing session behavior and leaves instant Tab cycling, streaming, Static scrollback with pinned input, and full resize/exit/non-TTY handling to `t8-1-2` through `t8-1-5`. The next executable action is to route Ink Tab input through the existing agent-switch controller without requiring Enter. No session implementation changes remain uncommitted; only pre-existing operator/support files remain dirty.

## 2026-08-13 — Instant Tab agent cycling

Decision: complete `t8-1-2` by routing Ink's bare Tab event into the existing `/tab` controller path rather than duplicating agent switching inside React. This preserves agent construction, active-state mutation, status rendering, the `/tab` compatibility command, and non-TTY behavior while changing the real interactive UX to cycle immediately without Enter. The compiled PTY regression now sends a bare tab byte, waits for the persistent frame to show Architect, and exits, proving the keybinding in the real compiled terminal path. README and `USER_TESTING.md` now expose immediate Tab cycling without claiming streaming, Static scrollback, or full resize/exit handling. Typecheck, lint, focused PTY/TUI tests, and all 226 tests passed. Phase 8 advances to `t8-1-3` with 27 tasks remaining; v0.1.1 is not released.

## 2026-08-13 — Instant Tab task handoff summary

Phase 8 is merged with 27 tasks remaining and active at `m8-1` / `t8-1-3`; v0.1.1 has not been released. This session completed and committed `t8-1-2` as `3110f5d` after a compiled real-PTY bare-Tab check, typecheck, lint, and all 226 tests passed. Tab routes through the existing `/tab` controller, so the compatibility command and non-TTY behavior remain unchanged. The next executable action is to trace the backend response interfaces before adding token-by-token assistant streaming to `SessionViewStore`. No session implementation changes remain uncommitted; only pre-existing operator/support files remain dirty.

## 2026-08-13 — Real interactive assistant streaming

Decision: complete `t8-1-3` with an explicit operator-approved exception to Phase 8's rendering-only scope after reconnaissance proved LOOM had no existing streaming seam: `runPrompt()` always waited for a completed JSON response and `BaseAgent.runTurn()` exposed only a completed result. The prompt gateway now supports bounded OpenAI-compatible SSE with `[DONE]` enforcement, usage collection, fixed malformed/truncated/oversized errors, a 1 MiB event bound, and a 16 MiB accumulated-content bound. `BaseAgent.runTurn()` retains its completed result contract and adds optional `AgentTurnOptions.onTextDelta`; all built-in agents project only plain assistant text or the decoded top-level JSON-envelope content, withholding tool arguments. Persistent TTY sessions render statefully redacted deltas in a dedicated live assistant region, clear stale state on failure, and commit the completed turn afterward. Scripted/non-TTY and `--prompt` paths retain completed JSON behavior. Split-sensitive-value tests, envelope-isolation tests, SSE integration/failure tests, typecheck, lint, all 234 tests, and independent security re-review passed with no critical/high findings; the pre-existing missing transport timeout remains medium hardening debt. README and tracked `SPEC.md` document streaming and the interface/scope exception. Phase 8 advances to `t8-1-4` with 26 tasks remaining; v0.1.1 is not released. The untracked `.loom/SPEC.md` duplicate still needs operator-controlled reconciliation rather than being silently absorbed into this commit.

## 2026-08-13 — Interactive streaming task handoff summary

Phase 8 is merged with 26 tasks remaining and active at `m8-1` / `t8-1-4`; v0.1.1 has not been released. This session completed and committed the operator-authorized SSE capability exception for `t8-1-3` as `658390d`; typecheck, lint, all 234 tests, and security re-review passed with no critical/high findings. Interactive TTY sessions stream safe assistant deltas while completed agent results, tool execution, accounting, and non-TTY/`--prompt` behavior remain intact. The next executable action is to move completed output into Ink `<Static>` while keeping the live assistant region and input/status areas dynamic. The untracked `.loom/SPEC.md` duplicate remains operator-controlled reconciliation debt; no session implementation changes remain uncommitted, and only pre-existing operator/support files remain dirty.

## 2026-08-14 — Static scrollback and live frame separation

Decision: complete `t8-1-4` by making Ink `<Static>` a sibling of the entire live frame rather than nesting completed output inside the dynamic content box. Completed output is now emitted once above the persistent tab strip, streaming assistant region, status bar, and input line, so growing history no longer participates in live-frame redraws or displaces those controls. The session controller, agent switching, tool execution, conversation history, streaming state, and non-TTY paths were not changed. A real component render test verifies completed-output ordering and retention of every live region; the compiled PTY regression, typecheck, lint, and all 235 tests passed. README now reflects Static scrollback while leaving resize and exit handling pending. Phase 8 advances to `t8-1-5` with 25 tasks remaining; v0.1.1 is not released.

## 2026-08-14 — Static scrollback task handoff summary

Phase 8 is merged with 25 tasks remaining and active at `m8-1` / `t8-1-5`; v0.1.1 has not been released. This session completed and committed `t8-1-4` as `37137fa` after typecheck, lint, the compiled PTY regression, focused plan/render tests, and all 235 tests passed. Completed history now uses Ink `<Static>` while the streaming assistant, status, and input remain in the live frame; existing controller and non-TTY behavior remain covered and unchanged. The next executable action is to read the current Ink session shutdown and input paths before implementing resize, Ctrl+C/Esc exit, and explicit non-TTY fallback handling. The untracked `.loom/SPEC.md` duplicate remains operator-controlled reconciliation debt, and only pre-existing operator/support files remain dirty.

## 2026-08-14 — Complete persistent Ink session lifecycle

Decision: complete `t8-1-5` and milestone `m8-1` without adding backend capability. Confirmed-TTY sessions now force Ink's interactive resize and alternate-screen behavior, disable Ink's independent Ctrl+C termination, and route Ctrl+C or Escape through an idempotently closable async input queue so the controller exits and restores the terminal normally. Non-TTY detection now accepts redirected regular files, FIFOs, and Bun's socket-backed child stdin; readline remains non-terminal-aware when stdout is redirected, while scripted input and `--prompt` retain their existing completed-output paths. The compiled binary passed real PTY resize followed by continued agent switching, Escape and Ctrl+C exits, alternate-screen enter/restore checks, piped `/exit`, and one-shot `--prompt`; typecheck, lint, focused tests, and all 236 tests passed. README and `USER_TESTING.md` now describe lifecycle behavior and the remaining operator terminal checks. Phase 8 advances to priority milestone `m8-2` / `t8-2-1` with 24 tasks remaining; v0.1.1 is not released, and final Phase 8 operator acceptance remains pending.

## 2026-08-14 — Persistent Ink lifecycle handoff summary

Phase 8 is merged with 24 tasks remaining and active at priority milestone `m8-2` / `t8-2-1`; milestone `m8-1` is complete and v0.1.1 has not been released. This session completed and committed `t8-1-5` as `c287821` after real compiled-PTY resize, Escape, Ctrl+C, and terminal-restoration checks; compiled piped-input and `--prompt` checks; typecheck; lint; and all 236 tests passed. No backend capability or controller behavior was added. Final Phase 8 operator terminal acceptance remains pending. The next executable action is to trace existing slash-command parsing and built-in/installed agent registries before designing the popup data model. The untracked `.loom/SPEC.md` duplicate remains operator-controlled reconciliation debt, and only pre-existing operator/support files remain dirty.

## 2026-08-14 — Inline slash-command discovery popup

Decision: complete `t8-2-1` with one typed slash-command registry shared by the popup and existing controller parser. Typing `/` as the first input character now renders an inline themed list beside the input with one entry for each currently available built-in agent plus `/tab`, `/agents`, `/exit`, and `/quit`; the list intentionally remains unfiltered and non-selectable until `t8-2-2` and `t8-2-3`. Reconnaissance confirmed LOOM has no installed-agent loading registry, so adding one would violate Phase 8's no-new-capability boundary; no installed-agent entries were fabricated, and future registry capability can supply them through the popup's typed entry interface. A compiled real PTY proved the popup appears on `/` and clears on Backspace before resize, agent cycling, and exit continued normally. Typecheck, lint, all 239 tests, and focused review passed with no critical/high findings. README and `USER_TESTING.md` document only current discovery behavior. Phase 8 advances to `t8-2-2` with 23 tasks remaining; v0.1.1 is not released, and final operator terminal acceptance remains pending.

## 2026-08-14 — Slash discovery task handoff summary

Phase 8 is merged with 23 tasks remaining and active at priority milestone `m8-2` / `t8-2-2`; milestone `m8-1` is complete and v0.1.1 has not been released. This session completed and committed `t8-2-1` as `c8b594d` after compiled real-PTY popup verification, typecheck, lint, all 239 tests, and review passed. The popup and controller share typed command literals, only current built-in agents are listed because no installed-agent runtime exists, and filtering/selection/Escape-close remain pending. Final Phase 8 operator acceptance remains pending. The next executable action is to add a pure tested popup-entry filter and connect it to live input. The untracked `.loom/SPEC.md` duplicate remains operator-controlled reconciliation debt, and only pre-existing operator/support files remain dirty.

## 2026-08-14 — Live slash-command filtering

Decision: complete `t8-2-2` with case-insensitive executable-command prefix filtering rather than description matching or aliases, which keeps future selection deterministic. `/` shows every entry, `/agent ` shows agent choices, `/agent t` narrows to Tester, and unmatched prefixes retain the popup with an explicit no-match message. The filter is a pure typed function connected directly to `SessionApp` input state, so every character and Backspace update the visible entries without changing controller behavior. A compiled real PTY entered `/agent t`, observed the Tester entry, cleared the input, and continued through resize, agent cycling, and exit. Typecheck, lint, and all 241 tests passed. Review returned no applicable findings; its claimed case sensitivity and timeout-based unit test did not exist, and Escape-close remains intentionally reserved for `t8-2-4`. README and `USER_TESTING.md` document current filtering without claiming selection. Phase 8 advances to `t8-2-3` with 22 tasks remaining; v0.1.1 is not released, and final operator terminal acceptance remains pending.

## 2026-08-14 — Slash filtering task handoff summary

Phase 8 is merged with 22 tasks remaining and active at priority milestone `m8-2` / `t8-2-3`; milestone `m8-1` is complete and v0.1.1 has not been released. This session completed and committed `t8-2-2` as `8fe9eb1` after compiled real-PTY filtered-popup verification, typecheck, lint, and all 241 tests passed. Filtering is case-insensitive and command-prefix-only, unmatched input remains visible as a no-match state, and selection/Escape-close remain pending. Final Phase 8 operator acceptance remains pending. The next executable action is to trace Ink 7 keyboard and mouse support before designing selected-index state and Enter-first acceptance. The untracked `.loom/SPEC.md` duplicate remains operator-controlled reconciliation debt, and only pre-existing operator/support files remain dirty.

## 2026-08-14 — Slash popup keyboard selection

Decision: complete `t8-2-3` with a visibly marked, wraparound popup selection controlled by Up and Down; Enter submits the selected registry command directly, while bare Tab retains its existing global agent-cycle behavior. Selection uses a synchronous ref alongside React state because Ink may deliver adjacent terminal input before a rerender. Ink 7.1 exposes keyboard input but no mouse-event API, so the task's conditional click path is unsupported by the installed renderer and no dependency or backend capability was added. A compiled real PTY opened the popup, moved Down to Architect, accepted with Enter, then proved bare Tab still cycled to Tester before resize and clean exit; piped input and `--prompt` remained covered. Typecheck, lint, all 243 tests, and focused review passed with no critical/high findings. README and `USER_TESTING.md` document keyboard selection and preserve final operator terminal acceptance as pending. Phase 8 advances to `t8-2-4` with 21 tasks remaining; v0.1.1 is not released.

## 2026-08-14 — Slash popup selection handoff summary

Phase 8 is merged with 21 tasks remaining and active at priority milestone `m8-2` / `t8-2-4`; milestone `m8-1` is complete and v0.1.1 has not been released. This session completed and committed `t8-2-3` as `faec558` after a compiled real-PTY Up/Down/Enter selection check, preserved bare-Tab cycling, typecheck, lint, all 243 tests, and focused review passed. Ink 7.1 has no mouse-event API, so no click capability or dependency was added. Final Phase 8 operator terminal acceptance remains pending. The next executable action is to read the current `SessionApp` exit/input ordering and focused tests before making Escape close only an open popup. The untracked `.loom/SPEC.md` duplicate remains operator-controlled reconciliation debt; all task-owned changes are committed, and only pre-existing operator/support files remain dirty.

## 2026-08-14 — Context-sensitive popup Escape

Decision: complete `t8-2-4` and milestone `m8-2` by handling Escape against synchronous popup-dismissal state before the existing session-exit path. Escape closes an open slash popup without submitting or clearing input; a subsequent text edit reopens live discovery for the preserved slash prefix, while a second Escape or Escape with no popup still exits and Ctrl+C remains an unconditional exit. No controller, backend, or non-TTY path changed. A compiled real PTY proved the popup closed while `/agent s` remained, typing `e` reopened the Security match, Enter selected it, bare Tab and resize still worked, and outside-popup Escape restored the terminal; piped input and `--prompt` remained covered. Typecheck, lint, all 244 tests, and planner validation passed. README and `USER_TESTING.md` document the context-sensitive Escape behavior. Phase 8 advances to `m8-3` / `t8-3-1` with 20 tasks remaining; v0.1.1 is not released, and final operator terminal acceptance remains pending.

## 2026-08-14 — Popup Escape handoff summary

Phase 8 is merged with 20 tasks remaining and active at `m8-3` / `t8-3-1`; milestones `m8-1` and `m8-2` are complete, and v0.1.1 has not been released. This session completed and committed `t8-2-4` as `bca494b` after compiled real-PTY popup dismissal/input-preservation/outside-Escape verification, typecheck, lint, all 244 tests, planner validation, and review passed. No controller, backend, or non-TTY behavior changed. Final Phase 8 operator terminal acceptance remains pending. The next executable action is to read `runAgentPrompt` and `SessionViewStore` streaming transitions before designing the pre-first-token thinking indicator. The untracked `.loom/SPEC.md` duplicate remains operator-controlled reconciliation debt; all task-owned changes are committed, and only pre-existing operator/support files remain dirty.

## 2026-08-14 — Pre-stream thinking indicator

Decision: complete `t8-3-1` with an interactive-only, agent-specific animated-dot Thinking state set synchronously before `runTurn()`. The state clears on the first raw text delta before streaming redaction buffering, preventing sensitive-value buffering from extending the thinking phase, and also clears on completion or failure. The animation is local Ink component state with no new dependency, backend capability, controller behavior, or non-TTY output change. A compiled real PTY submitted a normal prompt and observed Thinking before clean exit; focused store/component tests verify synchronous state transitions and animation frames. Typecheck, lint, all 245 tests, and planner validation passed. README and `USER_TESTING.md` document Thinking without claiming the distinct tool-running state reserved for `t8-3-2`. Phase 8 advances to `t8-3-2` with 19 tasks remaining; v0.1.1 is not released, and final operator terminal acceptance remains pending.

## 2026-08-14 — Thinking indicator handoff summary

Phase 8 is merged with 19 tasks remaining and active at `m8-3` / `t8-3-2`; milestones `m8-1` and `m8-2` are complete, and v0.1.1 has not been released. This session completed and committed `t8-3-1` as `2956a69` after compiled real-PTY Thinking verification, focused lifecycle/animation tests, typecheck, lint, all 245 tests, planner validation, and review passed. Thinking clears on the first raw delta before redaction buffering, completion, or failure; non-TTY behavior is unchanged. Final Phase 8 operator terminal acceptance remains pending. The next executable action is to read `BaseAgent.runTurn()` tool execution and `AgentTurnOptions` for an existing tool-running rendering seam, stopping if the task would require out-of-scope backend capability. The untracked `.loom/SPEC.md` duplicate remains operator-controlled reconciliation debt; all task-owned changes are committed, and only pre-existing operator/support files remain dirty.

## 2026-08-14 — Distinct tool-running indicator

Decision: complete `t8-3-2` and milestone `m8-3` through a narrow renderer-only `AgentTurnOptions` lifecycle event around existing tool execution in all four built-in agents. The shared helper reports start and finish in `finally` with only a validated tool count; it never exposes names, arguments, results, or content and adds no tool, backend, or transport capability. Persistent Ink maps the event to a warning-colored, animated active-agent `Running N tool(s)` state distinct from Thinking, preserving streamed assistant text and clearing on success or failure. Root `SPEC.md` section 6 and its changelog record the interface extension before implementation. A compiled real PTY used a local OpenAI-compatible fixture and safe generated file-writer call to observe `Running 1 tool`; focused helper, concrete-agent, store, and component tests cover zero tools, success, failure, count validation, and rendering. Typecheck, lint, all 251 tests, and planner validation passed; non-TTY behavior remains unchanged. README and `USER_TESTING.md` document the state without exposing tool data. Phase 8 advances to `m8-4` / `t8-4-1` with 18 tasks remaining; v0.1.1 is not released, and final operator terminal acceptance remains pending.

## 2026-08-14 — Tool-running indicator handoff summary

Phase 8 is merged with 18 tasks remaining and active at `m8-4` / `t8-4-1`; milestones `m8-1` through `m8-3` are complete, and v0.1.1 has not been released. This session completed and committed `t8-3-2` as `bbf5e2d` after compiled real-PTY `Running 1 tool` verification, focused lifecycle/agent/store/component tests, typecheck, lint, all 251 tests, planner validation, and review passed. The renderer receives only tool status/count, no arguments, results, names, or content; non-TTY behavior is unchanged. Final Phase 8 operator terminal acceptance remains pending. The next executable action is to read the current `<Static>` scrollback rendering and output-state representation before visually receding completed turns. The untracked `.loom/SPEC.md` duplicate remains operator-controlled reconciliation debt; all task-owned changes are committed, and only pre-existing operator/support files remain dirty.
